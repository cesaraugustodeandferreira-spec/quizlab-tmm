...
export function ProfessorShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <HeaderContext.Provider value={setHeader}>
      <div className="mx-auto flex w-full max-w-[1400px] gap-4 p-2.5 sm:p-3">
        <nav
        ...
        <nav
          aria-label="Navegação principal"
          className="sticky top-3 hidden h-[calc(100dvh-24px)] w-14 shrink-0 flex-col items-center justify-between rounded-[14px] border border-line bg-surface py-3 shadow-[0_8px_30px_rgba(0,0,0,0.35)] sm:flex"
        >
